	...
	public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (!isset($this->request->get['module_id'])) {
                $this->model_extension_module->addModule('occountdown', $this->request->post);
            } else {
		...
		$data['entry_showwishlist'] = $this->language->get('entry_showwishlist');
		$data['entry_showcompare'] = $this->language->get('entry_showcompare');
		...
		$data['entry_usethumbslide'] 	= $this->language->get('entry_usethumbslide');
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/occountdown', 'token=' . $this->session->data['token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/occountdown', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}
		...
		} elseif (!empty($module_info) && isset($module_info['showprice'])) {
		...
		} elseif (!empty($module_info) && isset($module_info['showdes'])) {
		...
		} elseif (!empty($module_info) && isset($module_info['showaddtocart'])) {
		...
		} elseif (!empty($module_info) && isset($module_info['showwishlist'])) {
		...
		} elseif (!empty($module_info) && isset($module_info['showcompare'])) {
		...
		if (isset($this->request->post['usethumbslide'])) {
			$data['usethumbslide'] = $this->request->post['usethumbslide'];
		} elseif (!empty($module_info)) {
			$data['usethumbslide'] = $module_info['usethumbslide'];
		} else {
			$data['usethumbslide'] = 0;
		}

		if(isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/occountdown', 'token=' . $this->session->data['token'] , true). '&module_id='.$this->request->get['module_id'];
		} else {
			$data['action'] = $this->url->link('extension/module/occountdown', 'token=' . $this->session->data['token'] , true);
		}
        ...
        $this->response->setOutput($this->load->view('extension/module/occountdown', $data));
	}

	public  function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/occountdown')) {

...
$email_subject = "Быстрый заказ";
$email_body = "Вы получили новый заказ. \n\n".
" Товар: ".$foproduct." (crazymol.com/index.php?route=product/product&product_id=".$product_id.") \n ".
" Цена: $foprice \n ".
" $fomodel \n ".
" $fostock \n\n".
" Имя покупателя: $name \n ".
...
mail($data['email'],$email_subject,$email_body,$headers);



//для модуля bestseller
$db_product_id = $this->db->query("SELECT product_id FROM bestsell_product WHERE product_id=" . (int)$product_id)->row['product_id'];

if($db_product_id){
	$this->db->query("UPDATE bestsell_product SET sells=(sells+1), date_modified=NOW() WHERE product_id=" . (int)$product_id);
}else{
	$this->db->query("INSERT INTO bestsell_product SET product_id=" . (int)$product_id.", sells=1, date_modified=NOW()");
}
